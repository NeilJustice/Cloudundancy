...
{
   const CloudundancyIni cloudundancyIni = _cloudundancyIniFileReader->ReadIniFile(sevenZipModeIniInputFilePath);
   ...
   const fs::path& destinationFolderPath) const
{
   ...
   while (true)
   {
      ...
      _caller_CopyNestedFileToFolder->CallConstMemberFunction(
   ...
   const Utils::FileCopyResult& fileCopyResult, const fs::path& destinationFolderPath) const
{
   ...
   {
      _console->WriteLineColor("Copied", Utils::Color::Green);
   }
   else
   {
