...
#pragma warning(pop)
   ...
   inline pair<size_t, shared_ptr<LeafPattern>> ArgumentLeafPattern::SingleMatch(const vector<shared_ptr<Pattern>>& left) const
   {
   ...
   inline pair<size_t, shared_ptr<LeafPattern>> CommandArgumentLeafPattern::SingleMatch(const vector<shared_ptr<Pattern>>& left) const
   {
      pair<size_t, shared_ptr<LeafPattern>> ret{};
      for (size_t i = 0, size = left.size(); i < size; ++i)
      {
...
#pragma warning(disable: 4868) // compiler may not enforce left - to - right evaluation order in braced initializer list
#endif
   inline pair<size_t, shared_ptr<LeafPattern>> OptionLeafPattern::SingleMatch(const vector<shared_ptr<Pattern>>& left) const
   {
      ...
      return { distance(left.begin(), thematch), dynamic_pointer_cast<LeafPattern>(*thematch) };
   }
#ifdef _WIN32
#pragma warning(pop)
#endif
   ...
   static vector<string> ParseSection(const string& name, const string& source)
   {
      ...
      regex const re_section_pattern
      {
      ...
      vector<string> ret;
      for_each(sregex_iterator(source.begin(), source.end(), re_section_pattern),
         sregex_iterator(),
         [&](const smatch& match)
         {
